...
using namespace std;
	class conne{
		...
		char* conectar(conectar,"localhost","root", "42895097", "db_empresa", 3306, NULL,0 ){
			
				if(!(obj=mysql_init(0)))
					{
						return "Error Create Objeto";
					}
					if(!mysql_real_connect("localhost","root", "42895097", "db_empresa", 3306, NULL,0 )
					{
	...
	class clempresa{
		public:
			...
			void mostrar(){
				   ...
				   if(!mysql_real_connect(objDatos,"localhost", "root", "", "db_empresa", 3306, NULL, 0)) {
				      cout<<"Error BD"<<MYSQL_PORT<<" Error: "<<mysql_error(objDatos)<<endl;
				      mysql_close(objDatos);
				      rewind(stdin);
				      getchar();
				   }
				   if(mysql_select_db(objDatos, "db_empresa")) {
